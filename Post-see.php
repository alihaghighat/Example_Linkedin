        ...
        if (count($getPost) > 0) {
            if (isset($_REQUEST['action'])) {
                ...
                if ($_REQUEST['action'] == 'like') {
                    ...
                    if ($iduser != $postiduser) {
                        $getuser = getrecord("tbluser", "iduser=$iduser");
                        $notfcations = $getuser[0]['name'] . ' ' . $getuser[0]['lastname'] . 'پست شما راپسندید';
                        addrecored("tblNotification", array("iduser" => $postiduser, "kind" => 3, "idkind" => $iduser, "vlaue" => $notfcations, "status" => 0));
                    }
                    echo '<script>location.replace("../Post-see?idpost=' . $idpost . '")</script>';
                }
                if ($_REQUEST['action'] == 'like-comment') {
                    if (isset($_REQUEST['idcomment'])) {
                        ...
                        if ($getcountcommentlik[0]['Tedad'] > 0) {
                            ...
                            $idcommentrplay = $getcountcommentlik[0]['iduser'];
                ...
                if (isset($_REQUEST['comment']) and $_REQUEST['comment'] != '') {
                    ...
                    addrecored("tblcomment", array("text" => $comment, "iduser" => $iduser, "idpost" => $idpost));
                    if ($iduser != $postiduser) {
                        $getuser = getrecord("tbluser", "iduser=$iduser");
                        $notfcations = $getuser[0]['name'] . ' ' . $getuser[0]['lastname'] . 'برای پست شما کامنت گذاشت';
                        addrecored("tblNotification", array("iduser" => $postiduser, "kind" => 4, "idkind" => $iduser, "vlaue" => $notfcations, "status" => 0));
                    }
                    echo '<script>location.replace("../Post-see?idpost=' . $idpost . '")</script>';
                } else {
                }
...
<script>
    ...
    function chatList(idpost) {

        $.ajax({
            url: 'send-post-popup.php',
            type: 'post',
            data: {
                'idpost': idpost
            },
    ...
    <div class="layout-px-spacing">
        ...
        <div class="page-header">
            <div class="page-title">
                <h3>Post of <?php $array = array_map('strval', explode(' ', $getPost[0]['text']));
                            echo $array[0] . ' ' . $array[1] . ' ' . $array[3]; ?> ...</h3>
            </div>
        </div>
            ...
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 layout-spacing">
                ...
                foreach ($getPost as $key) {
                    ...
                    echo ' <div class="card-body" >
                                ...
                                <button class="btn btn-info" onclick="chatList(' . $idpost . ')">ارسال پست</button>
